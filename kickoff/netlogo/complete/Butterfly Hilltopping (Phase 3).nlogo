...
to setup
  clear-all                                        ; Clear agents, plot, etc.
  setup-terrain                                    ; Set patch elevation.
  setup-butterflies                                ; Create & place butterflies.
  reset-ticks                                      ; Start simulation clock.
end

to setup-terrain
  let :center-x ((max-pxcor + min-pxcor) / 2)      ; X center of world.
  let :center-y ((max-pycor + min-pycor) / 2)      ; Y center of world.
  ask patches [
    ; Peak at center, elevation decreasing with distance, with randomness.
...
to setup-butterflies
  ...
  create-butterflies num-butterflies [
    ...
    set size 10                                    ; Make shapes visible.
  ]
end
...
to-report average-local-relative-density
  ...
  report ((mean [count butterflies in-radius :radius] of butterflies)
...
920
295
Average Local Relative Density
Ticks
Neighborhood Density / Population
0.0
10.0
0.0
0.01
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot average-local-relative-density\n"
